...
import { Loader2, CheckCircle2, AlertCircle, Send, FileText, User, Mail, AtSign, MessageSquare, FileCode, Calendar } from 'lucide-react';
...
const Field = ({ label, icon: Icon, textarea, ...props }: { label: string; icon: React.ElementType; textarea?: boolean;[key: string]: any }) => (
    <div>
        <div style={labelStyle}><Icon size={12} />{label}</div>
        <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', top: textarea ? '11px' : '50%', left: '11px', transform: textarea ? 'none' : 'translateY(-50%)', color: 'rgba(255,140,0,0.55)', pointerEvents: 'none' }}>
                <Icon size={14} />
            </div>
            {textarea
                ? <textarea {...props} style={{ ...inputBase, resize: 'none', lineHeight: 1.5 }} rows={3}
...
const HtmlEmail = () => {
    ...
    const [htmlFiles, setHtmlFiles] = useState<FileData[]>([]);
    ...
    useEffect(() => {
        api.get('/upload?limit=100').then(({ data }) => {
            ...
            setHtmlFiles(uploadedFiles.filter((f: any) => f.fileType === 'HTML'));
        }).catch(console.error);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload: any = { type: 'HTML', ...formData, toEmails: emailsArray };
    ...
    return (
        <div className="max-w-2xl mx-auto pb-10 space-y-5">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-2xl font-extrabold text-white">HTML Based Email</h1>
                    <p className="text-xs" style={{ color: 'rgba(255,200,120,0.6)' }}>Send an HTML template as email campaign</p>
                </div>
            </div>
                ...
                <div className="p-6 space-y-4">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2">
                                <Field label="Disclaimer (use ##disclaimer## in HTML)" icon={FileText} type="text" name="disclaimer" value={formData.disclaimer} onChange={handleChange} placeholder="Disclaimer text..." />
                            </div>
                        </div>

                        <SelectField label="Select HTML Template" icon={FileCode} name="fileUrl" required value={formData.fileUrl} onChange={handleChange}>
                            <option value="" disabled style={{ background: '#1a1035' }}>— Choose HTML File —</option>
                            {htmlFiles.map(f => <option key={f._id} value={f.fileUrl} style={{ background: '#1a1035' }}>{f.fileName}</option>)}
                        </SelectField>
                        {htmlFiles.length === 0 && <p className="text-xs" style={{ color: '#FB8500' }}>No HTML files found. <a href="/upload" className="underline">Upload one first.</a></p>}
