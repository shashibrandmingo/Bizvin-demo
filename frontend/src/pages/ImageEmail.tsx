...
const ImageEmail = () => {
    ...
    const [formData, setFormData] = useState({
        fromName: '', fromEmail: '', emailConfigIds: [] as string[], delay: 0, replyTo: '', toName: '', toEmails: '',
        subject: '', disclaimer: '', disclaimerPosition: '', linkOnImage: '', fileUrl: ''
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormData({ fromName: '', fromEmail: '', emailConfigIds: [], delay: 0, replyTo: '', toName: '', toEmails: '', subject: '', disclaimer: '', disclaimerPosition: '', linkOnImage: '', fileUrl: '' });
            setScheduledAt('');
        } catch (err: any) {
                ...
                <div className="p-6 space-y-4">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <Field label="Disclaimer Text" icon={FileText} type="text" name="disclaimer" value={formData.disclaimer} onChange={handleChange} placeholder="Disclaimer text..." />
                            <SelectField label="Disclaimer Position" icon={AlignCenter} name="disclaimerPosition" value={formData.disclaimerPosition} onChange={handleChange}>
                                <option value="" disabled style={{ background: '#1a1035' }}>— Select Position —</option>
                                <option value="Bottom" style={{ background: '#1a1035' }}>Bottom</option>
                                <option value="Top" style={{ background: '#1a1035' }}>Top</option>
                            </SelectField>
                            <div className="md:col-span-2">
                                <Field label="Link on Image (URL)" icon={Link} type="url" name="linkOnImage" value={formData.linkOnImage} onChange={handleChange} placeholder="https://example.com" />
                            </div>
                        </div>
